...
interface EventoDetailModalProps {
    ...
    evento: Evento | null;
    isPastor: boolean;
    onAprobar?: (id: string, estado: 'aprobado' | 'rechazado') => Promise<boolean>;
...
export const EventoDetailModal: React.FC<EventoDetailModalProps> = ({
    ...
    const handleAprobar = async (estado: 'aprobado' | 'rechazado') => {
        if (!onAprobar) return;
        setProcesando(true);
        await onAprobar(evento.id_evento, estado);
        setProcesando(false);
        onClose();
    };
