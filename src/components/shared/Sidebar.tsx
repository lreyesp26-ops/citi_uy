...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      {/* Mobile Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-30 w-72 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out lg:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
          ...
          {links.map((link) => {
            ...
            return (
              ...
              >
                {({ isActive }) => (
                  <>
                    <Icon className={`mr-3.5 h-5 w-5 flex-shrink-0 transition-colors ${isActive ? 'text-red-700' : 'text-gray-400'
                      }`}
                      aria-hidden="true"
                    />
                    {link.name}
