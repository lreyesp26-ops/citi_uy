...
import { UserCheck, Users, Search, Shield, Mail, Phone, MapPin, Calendar, Briefcase } from 'lucide-react';
...
const PersonaCard: React.FC<{ persona: PersonaConMinisterios }> = ({ persona }) => {
            ...
            <div className="p-5 space-y-4">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <span
                            className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-xs font-semibold text-white"
                            style={{ backgroundColor: accentColor }}
                        >
                            ...
                            {persona.ministerios_asignados.map((m, i) => (
                                <span
                                    ...
                                    style={{
                                        ...
                                        color: m.estado_activo ? m.color : '#9ca3af',
                                    }}
                                >
                                    <span
...
export const LideresPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                // Traer líderes y pastores con sus ministerios
                const { data: personasData } = await supabase
                    .from('personas')
                    .select(`
                        id_persona, nombres, apellidos, rol,
                        correo_electronico, celular, direccion,
                        fecha_nacimiento, genero, profesion,
                        estado_activo, created_at
                    `)
                    .in('rol', ['lider', 'pastor'])
                    .order('apellidos');
                ...
                const { data: lideresData } = await supabase
                    ...
                    .select(`
                        id_persona,
                        ministerios (nombre, color, estado_activo)
                    `);

                // Mapear ministerios por persona
                const ministeriosPorPersona: Record<string, { nombre: string; color: string; estado_activo: boolean }[]> = {};
                (lideresData || []).forEach((ml: any) => {
                    if (!ministeriosPorPersona[ml.id_persona]) {
                        ministeriosPorPersona[ml.id_persona] = [];
                    }
                    if (ml.ministerios) {
                        ministeriosPorPersona[ml.id_persona].push({
                            nombre: ml.ministerios.nombre,
                            color: ml.ministerios.color,
                            estado_activo: ml.ministerios.estado_activo,
                        });
                    }
                });
        ...
        <div className="space-y-8">
            ...
            <section>
                ...
                {pastores.length === 0 ? (
                    <EmptyState
                        ...
                        title="Sin pastores registrados"
                        description="No hay pastores en el sistema aún."
                        className="py-8"
                    />
                ) : (
            ...
            <section>
                ...
                {lideres.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {lideres.map(p => <PersonaCard key={p.id_persona} persona={p} />)}
                    </div>
                )}
            </section>
        </div>
